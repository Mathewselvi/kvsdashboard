...
const TABS = [
  { id: 'purchases', label: 'Drying Records',  icon: ShoppingCart },
...
const CardamomStore = () => {
  ...
  // ── Table headers ──────────────────────────────────────────────────────────
  const tableHeaders = {
    purchases: ['Date', 'Farmer Details', 'Raw Wt (KG)', 'Drying Rate/KG', 'Drying Charge', 'Advance Paid', 'Balance Due', 'Status', 'Actions'],
      ...
      case 'purchases': {
        ...
        return (
          <tr key={item._id} className="border-b border-genesis-border hover:bg-gray-50/50 transition-colors">
            ...
            {cell(
              <div>
                <span className="font-semibold text-genesis-textMain block">{item.sellerName}</span>
                {item.sellerContact && <span className="text-[11px] font-mono text-genesis-textSub block">{item.sellerContact}</span>}
                {item.sellerAddress && <span className="text-[11px] text-genesis-textSub block truncate max-w-[180px]">{item.sellerAddress}</span>}
              </div>
            )}
          ...
          <div key={item._id} className="p-4 space-y-2.5 bg-white">
            <div className="flex justify-between items-center">
              <div>
                ...
                {item.sellerContact && <p className="text-[12px] font-mono text-genesis-textSub">{item.sellerContact}</p>}
                {item.sellerAddress && <p className="text-[12px] text-genesis-textSub truncate max-w-[200px]">{item.sellerAddress}</p>}
            ...
            <div className="flex justify-between text-[13px]">
              <span className="text-genesis-textSub">Drying Charge</span>
              <span className="font-bold text-genesis-success">{fmt(item.totalAmount)}</span>
            </div>
            <div className="flex justify-between text-[13px]">
              <span className="text-genesis-textSub">Advance Paid</span>
        ...
        return (
          <>
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 gap-4">
              ...
              <div><label className={labelClass}>Drying Rate per KG (₹)</label><input type="number" name="ratePerKG" value={formData.ratePerKG || ''} required step="0.01" onChange={handleChange} className={inputClass} placeholder="0.00" /></div>
            </div>
            ...
            {totalAmt > 0 && (
              <div className="bg-gray-50 border border-genesis-border rounded-lg p-4 grid grid-cols-1 xs:grid-cols-3 gap-3">
                <div>
                  <p className="text-[10px] text-genesis-textSub uppercase font-mono tracking-wider mb-0.5">Drying Charge</p>
                  <p className="text-[15px] font-bold text-genesis-success">₹{totalAmt.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[10px] text-genesis-textSub uppercase font-mono tracking-wider mb-0.5">Advance Paid</p>
                  <p className="text-[15px] font-bold text-genesis-textSub">₹{advance.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[10px] text-genesis-textSub uppercase font-mono tracking-wider mb-0.5">Balance Due</p>
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-4">
        <div>
          ...
          <p className="text-[14px] text-genesis-textSub font-body mt-2">Track drying records, payments, expenses and salaries</p>
        </div>
        <button
          onClick={openAdd}
      ...
      {activeTab === 'purchases' && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-white p-6 rounded-xl border border-genesis-border shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
            <div className="p-3 bg-emerald-50 rounded-lg text-emerald-600">
              <Scale size={24} />
            </div>
            <div>
              <p className="text-[11px] font-bold text-genesis-textSub uppercase tracking-wider mb-0.5">Total Cardamom Dried</p>
              <h3 className="text-2xl font-display font-bold text-genesis-textMain">
                {data.reduce((s, r) => s + (r.rawWeightKG || 0), 0).toLocaleString('en-IN')} KG
              </h3>
            </div>
          </div>
          <div className="bg-white p-6 rounded-xl border border-genesis-border shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
            ...
            <div>
              <p className="text-[11px] font-bold text-genesis-textSub uppercase tracking-wider mb-0.5">Total Drying Charges</p>
              <h3 className="text-2xl font-display font-bold text-genesis-textMain">
                ₹{data.reduce((s, r) => s + (r.totalAmount || 0), 0).toLocaleString('en-IN')}
              </h3>
            </div>
          </div>
          <div className="bg-white p-6 rounded-xl border border-genesis-border shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
            ...
            <div>
              <p className="text-[11px] font-bold text-genesis-textSub uppercase tracking-wider mb-0.5">Total Balance Due</p>
              <h3 className="text-2xl font-display font-bold text-genesis-error">
                ₹{data.reduce((s, r) => {
                  const amt = r.totalAmount || 0;
                  const adv = r.advancePayment || 0;
                  const rem = r.remainingPaid || 0;
                  return s + Math.max(0, amt - adv - rem);
        ...
        <div className="fixed inset-0 bg-genesis-textMain/20 z-50 flex items-center justify-center backdrop-blur-sm p-4">
          <div className="bg-genesis-surface border border-genesis-border rounded-xl shadow-genesis w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-genesis-border flex justify-between items-center bg-white">
              <h3 className="text-[16px] font-display font-bold text-genesis-textMain">Record Drying Charge Payment</h3>
            ...
            <form onSubmit={handleReceiveSubmit} className="p-6 space-y-4">
              <p className="text-[14px] text-genesis-textSub mb-2">
                Recording payment from <strong className="text-genesis-textMain font-medium">
                  {selectedPurchase.sellerName || '—'}
